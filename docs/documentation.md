...
* `softmax(dims?: number[] | number): Tensor`: Returns a new tensor with axes reduced to their minimums. If `dims` is `undefined`, all axes will be reduced into a scalar. If `dims` is a number/number array, it will reduce dimensions at that/those positions. If `keepDims` is `true`, then the size-1 dimensions after reduction will be kept, discarded otherwise.
