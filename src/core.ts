...
export class Tensor {
    ...
    elementWiseABDAG(
        ...
        if (out.requiresGrad) {
            out.gradFn = () => {
                ...
                if (this.requiresGrad) Tensor.addGrad(this, thisGrad(selfWithGrad, otherWithGrad, outGrad));
                if (other.requiresGrad) Tensor.addGrad(other, otherGrad(selfWithGrad, otherWithGrad, outGrad));
            };
        }
    ...
    elementWiseSelfDAG(
        ...
        if (out.requiresGrad) {
            out.gradFn = () => {
                ...
                if (this.requiresGrad) Tensor.addGrad(this, thisGrad(selfWithGrad, outGrad));
            };
        }
    ...
    expand(newShape: number[]): Tensor {
        ...
        if (self.requiresGrad) {
            ...
            out.gradFn = () => {
                Tensor.addGrad(self, out.grad as Tensor);
            }
        }
    ...
    unfold(dim: number, size: number, step: number): Tensor {
        ...
        if (size <= 0 || step <= 0) throw new Error("size and step must be greater than 0");

        const dimSize = this.shape[dim];

        // Verify size against dimension size
        if (size > dimSize) throw new Error("size can not be greater than dimension size");
    ...
    // Tensor sort
    sort(dim = -1, descending = false): Tensor {
        if (dim < 0) { dim += this.shape.length }
    ...
    // Top-k sampling
    topk(k: number, dim = -1, largest = true): Tensor {
        if (dim < 0) { dim += this.shape.length }
    ...
    static reduceArg(
        ...
        const outputStrides = Tensor.getStrides(outputShape);
        const outputSize = tensor.numel / dimSize;
        const bestValues = new TypedArray[tensor.dtype](outputSize).fill(config.identity);
    ...
    // Tensor argmax
    argmax(dim: number, keepDim = false): Tensor {
        return Tensor.reduceArg(
            this,
            dim,
            keepDim,
            {
    ...
    // Tensor argmin
    argmin(dim: number, keepDim = false): Tensor {
        return Tensor.reduceArg(
            this,
            dim,
            keepDim,
    ...
    // Tensor (stable) logsumexp
    logSoftmax(dim: number = -1): Tensor {
        if (this.shape.length === 0) return this;
    ...
    selu(): Tensor {
        ...
        return this.elementWiseSelfDAG(
            (a) => scale * (a >= 0 ? a : alpha * Math.expm1(a)),
            (self, outGrad) => {
    ...
    celu(alpha = 1): Tensor {
        return this.elementWiseSelfDAG(
            (a) => a >= 0 ? a : alpha * (Math.expm1(a / alpha)),
            (self, outGrad) => {
    ...
    copysign(other: TensorValue | Tensor): Tensor {
        return this.elementWiseABDAG(
            other,
            (a, b) => Math.abs(a) * (Object.is(b, -0) || b < 0 ? -1 : 1),
            (self, other, outGrad) => outGrad.mul(self.sign().mul(other.sign())),
            (self, other, outGrad) => new Tensor(0)
        );
    }
    ...
    mm(other: TensorValue | Tensor): Tensor {
        ...
        if (out.requiresGrad) {
            out.gradFn = () => {
                ...
                if (this.requiresGrad) Tensor.addGrad(this, outGrad.mm(otherWithGrad.t()));
                if (other.requiresGrad) Tensor.addGrad(other, selfWithGrad.t().mm(outGrad));
            };
        }
    ...
    bmm(other: TensorValue | Tensor): Tensor {
        ...
        if (out.requiresGrad) {
            out.gradFn = () => {
                ...
                if (this.requiresGrad) Tensor.addGrad(this, outGrad.bmm(otherWithGrad.transpose(1, 2)));
                if (other.requiresGrad) Tensor.addGrad(other, selfWithGrad.transpose(1, 2).bmm(outGrad));
            };
        }
        ...
        if (isThis1D && isOther1D) {
            ...
            if (out.requiresGrad) {
                out.gradFn = () => {
                    ...
                    if (other.requiresGrad) Tensor.addGrad(other, selfWithGrad.transpose(-2, -1).matmul(outGrad));
                };
            }
    ...
    tensordot(other: TensorValue | Tensor, axes: number | [number, number] | [number[], number[]] = 2): Tensor {
        other = this.handleOther(other);
        ...
        // If axes is a pair of numbers or a pair of axes
            ...
            if (typeof axesA === "number") axesA = [axesA];
            if (typeof axesB === "number") axesB = [axesB];
        }
        ...
        for (let i = 0; i < axesA.length; i++) {
            if (this.shape[axesA[i]] !== other.shape[axesB[i]]) {
                throw new Error(
                    `Dimension mismatch: a.shape[${axesA[i]}]=${this.shape[axesA[i]]} ` +
    ...
    toString(): string {
        ...
        const formatNum = (n: number): string => {
            ...
            if (this.dtype.includes("int")) return n.toFixed(0);
            // For ints with float dtype
            if (Number.isInteger(n) && Math.abs(n) < 1e8) return n.toFixed(0) + ".";
            // For floats
            return n.toString();
        };
        ...
        const collectNumbers = (v: TensorValue): number[] => {
            if (typeof v === "number") return [v];
            return (v as TensorValue[]).flatMap(collectNumbers);
        };
        ...
        const formatNested = (v: TensorValue, depth: number): string => {
            if (typeof v === "number") {
                return formatNum(v).padStart(maxWidth);
            }

            const arr = v as TensorValue[];

            // Innermost dimension: format as single line [x, y, z]
            if (arr.length > 0 && typeof arr[0] === "number") {
                const elements = arr.map((x) =>
                    formatNum(x as number).padStart(maxWidth)
                );
